...
class CartController {
	...
	public String addDisc(@RequestParam("pid") Disc disc, @RequestParam("number") int number, @ModelAttribute Cart cart) {
        ...
        public String deleteDisc(@RequestParam("iid") Disc disc, @RequestParam("number") int number, @ModelAttribute Cart cart)
        {
           if(number <= 0)
           {
           ...
           number = number *(-1);
           CartItem cartItem = cart.addOrUpdateItem(disc, Quantity.of(number));
           
           if(cartItem.getQuantity().isLessThan(Quantity.of(1)))
           {
